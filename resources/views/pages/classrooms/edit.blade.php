...
@section('content')
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">Cập nhật lớp học</li>
        </ol>
    </nav>
    <div class="form-content shadow">
        <h6 class="font-weight-bold text-primary">Cập nhật lớp học</h6>
        <form class="needs-validation form-main" novalidate method="post" action="{{ route('classrooms.update', $data) }}"
              enctype="multipart/form-data">
            {{ csrf_field() }}
            @method('PUT')
            <div class="form-group">
                <label class="required" for="name">Tên lớp học</label>
                <input name="name" value="{{$data['name']}}" type="text" class="form-control" id="name" placeholder="Nhập tên lớp học"
                    required>
