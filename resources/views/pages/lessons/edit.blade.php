...
@section('content')
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">Cập nhật buổi học</li>
        </ol>
    </nav>
    <div class="form-content shadow">
        <h6 class="font-weight-bold text-primary">Cập nhật buổi học</h6>
        <form class="needs-validation form-main" novalidate method="POST"
            action="{{ route('lessons.update', ['id' => $data['id']]) }}" enctype="multipart/form-data">
            {{ csrf_field() }}
            @method('PUT')
            ...
            <div class="form-group">
                ...
                <input value={{ $data['start_time'] }} name="start_time" type="time" class="form-control" id="start_time"
            ...
            <div class="form-group">
                ...
                <input value={{ $data['date'] }} name="date" type="date" class="form-control" id="start_day"
            ...
            <div class="form-group">
                ...
                <select name="substitute_instructor" class="form-control" id="lecturer">
                    <option value="0">Chọn người hướng dẫn thay thế...</option>
                    @foreach ($lecturers as $lecturer)
                        <option value="{{ $lecturer['id'] }}"
                            {{ $lecturer['id'] == $data['substitute_instructor'] ? 'selected' : '' }}>
                            {{ $lecturer['full_name'] }}
                        </option>
                    @endforeach
                </select>
                ...
                <select name="schedule_id" class="form-control" id="schedule" required>
                    ...
                    @foreach ($schedules as $schedule)
                        <option value="{{ $schedule['id'] }}"
                            {{ $schedule['id'] == $data['schedule_id'] ? 'selected' : '' }}>
            ...
            <div class="form-group">
                ...
                <select name="lecturer_id" class="form-control" id="lecturer" required>
                    ...
                    @foreach ($lecturers as $lecturer)
                        <option value="{{ $lecturer['id'] }}"
                            {{ $lecturer['id'] == $data['lecturer_id'] ? 'selected' : '' }}>
                            {{ $lecturer['full_name'] }}
                        </option>
                    @endforeach
                </select>
